...
const ListItem = ({ no, item, selected = false, onClick, onClickActionCallback }: ListItem) => {
  ...
  return (
    <>
      <div className="flex items-center w-full cursor-pointer justify-between relative">
        <div className="py-3 px-4 flex justify-between w-full relative items-center" onClick={onClickItem}>
          <div className="flex items-center w-full">
            {no && <div className="-ml-1 mr-4 text-sm text-secondary w-2.5 text-center">{formatNo(no)}</div>}

            <ListImageWrapper>
              <CoverArt type={item.__model__} src={src} title={title} loading={loadingCover || loading} onClick={onClickCoverArt} />
              <SharedUnsharedIcon shared={(item as Storage).shared} classname="absolute -top-1 left-8" />
            </ListImageWrapper>

            <div className="text-left grow w-0 pr-5">
      ...
      <div className="pr-2">
        <ActionMenu item={item} onClickActionCallback={onClickActionCallback} />
      </div>
    </>
  );
};
